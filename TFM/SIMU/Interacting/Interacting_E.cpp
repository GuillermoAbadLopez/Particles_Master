...
int main(){
	...
	for (i=0; i<Nx; i++){
		...
		Rold[i]= (exp(-0.5*((i*delx-mx/3)/(0.5*sig))*( (i*delx-mx/3)/(0.5*sig) ))+exp(-0.5*((i*delx-mx*3.5/2)/(0.5*sig))*( (i*delx-mx*3.5/2)/(0.5*sig) )))/(sig*sqrt(2*3.14159265));
		Rold[i]= (exp(-0.5*((i*delx-mx/3)/(0.5*sig))*( (i*delx-mx/3)/(0.5*sig) ))+exp(-0.5*((i*delx-mx*3.5/2)/(0.5*sig))*( (i*delx-mx*3.5/2)/(0.5*sig) )))/(sig*sqrt(2*3.14159265));
	...
	for(t=0 ; t<Time ; t++){
		...
		for(i=0; i<Nx-1; i++){
			...
			Rnew[i+1]= - r*(Rold[i+1]-Rold[i])+ (1+e*Aold[i+1])*Rold[i+1];
